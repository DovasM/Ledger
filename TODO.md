...
- [ ] **A settlement does not touch your wallet either** — `settlements.transaction_id` exists and is
  always null: recording that Mike paid you 120 changes the group balance but no wallet gains the
  120. Same shape of gap as the one below, and probably the same fix
- [ ] **A shared expense and its transaction are still two separate entries** — recording that you paid 100 for a
  ...
  left the wallet. But nothing links the two yet, so it has to be typed twice. The next step is the Add/Edit
  transaction screens offering to record a split for the transaction being saved, and the shared expense filling
  in  when you were the payer
...
- [x] **Transfers are listed and can be removed** — WalletDetailsScreen gained a Transfers section showing every
  transfer on either side of that wallet: direction, the other wallet's name, the amount, the date and the note,
  with delete behind a confirmation and a New Transfer button beside it. Since `m9` the balance is derived, so
  removing one is just removing the row — both balances simply stop including it, and no report changes because a
  transfer was never income or spending. Note the list will be empty for data that came from a Money Manager
  import: those transfers are still not imported (separate item below)
...
- [x] **Recurring transaction scheduling is tested** — `applyDueRecurring` mixed calendar arithmetic with FFI writes, so the part that can actually be wrong could not be tested. The schedule moved to `data/RecurringSchedule.kt` as `planDueRecurring(items, today)`; the bridge just executes it. 14 tests: due today posts, due tomorrow does not, an item missed since December posts every occurrence it owes and each row is dated the day it was due (not all on today), every frequency advances by its own step, an unknown frequency falls back to monthly, a daily item posts once per missed day, a month-end date clamps to the 28th, a leap day advances safely, several items schedule independently, and an unreadable date is skipped rather than guessed
- [x] **Off-budget report filtering is tested** — `TransactionUiState.forReports` in 5 tests, including that the unfiltered list stays complete so editing and search can still reach an off-budget row
- [x] **Budget and transaction functionality is covered by tests** — 39 Kotlin tests over the budget maths (`./gradlew testDebugUnitTest`, no device needed: `ui/util` is Compose-free) plus 13 more Rust tests at the DB layer, 75 in total. Covers the everything-budget (daily share, rollover on and off, carry-over including the deficit case and the created-after guard, newest-wins, wallet scoping, off-budget exclusion), per-category budgets (weekly/monthly/yearly pacing, ratio and alert threshold, over-limit, tightest-category, case-insensitive matching, deleted category, independence), streaks, and multi-transaction behaviour (sums, month boundaries, per-wallet listing, validation, delete). Every expected number is derived by hand from a fixed clock in `BudgetFixtures.kt` rather than copied from the current output, and the suite was verified by injecting three regressions into StreakCalculator and confirming each was caught by the right test
- [x] **A category could be budgeted twice for the same period** — found by the new tests. `idx_budgets_unique` cannot catch it: its other column is NULL for a category budget and SQLite treats NULLs as distinct in a unique index, so every row looked unique to it. StreakCalculator builds one pace per budget, so the category would appear twice on BudgetsScreen and the tightest-category widget could pick either. `create_budget` now refuses it explicitly, the same way it refuses a second overall budget, and does the same for wallet-scoped budgets
