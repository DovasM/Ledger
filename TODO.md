...
- [ ] **Reports do not filter off-budget wallets** — the agreed behaviour is "show separately with a toggle", but `MonthlyStatistics`, `AnnualSummary`, `MonthlyReport`, `QuarterlyReport`, `CustomReport`, `CashFlowForecast`, `BudgetInsights` and `Dashboard` all still mix them in. Needs a shared preference plus a filtered transaction list the report screens consume
- [ ] **Off-budget wallets are not marked in WalletsList** — nothing on the list says an account is excluded, so the only way to tell is to open it
