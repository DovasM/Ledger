...
fn m7_carries_existing_balances_into_the_history() {
    ...
    assert_eq!(emergency.current_amount, 8500.0, "the saved money must survive the column being dropped");
    assert_eq!(emergency.target_amount, 10000.0);

    let opening = db.list_goal_contributions("g1".into()).unwrap();
    assert_eq!(opening.len(), 1);
    assert_eq!(opening[0].amount, 8500.0);
    assert_eq!(opening[0].kind, "opening");

    let empty = goals.iter().find(|g| g.id == "g2").unwrap();
    assert_eq!(empty.current_amount, 0.0);
    ...
    assert_eq!(car.remaining_amount, 3400.0, "what is still owed must come out identical");
    assert_eq!(car.total_amount, 5000.0);

    let paid = db.list_debt_payments("d1".into()).unwrap();
    assert_eq!(paid.len(), 1);
    assert_eq!(paid[0].amount, 1600.0, "the part already paid off becomes the opening entry");
    assert_eq!(paid[0].kind, "opening");

    let untouched = debts.iter().find(|d| d.id == "d2").unwrap();
    assert_eq!(untouched.remaining_amount, 1000.0);
...
fn m7_is_idempotent() {
    ...
    let db = uniffi_ledger::open_database(path.to_string_lossy().to_string());
    assert_eq!(db.list_goals().unwrap().iter().find(|g| g.id == "g1").unwrap().current_amount, 8500.0);
    drop(db);

    let reopened = uniffi_ledger::open_database(path.to_string_lossy().to_string());
    assert_eq!(
        reopened.list_goals().unwrap().iter().find(|g| g.id == "g1").unwrap().current_amount,
        8500.0,
...
fn a_fresh_database_migrates_all_the_way_and_works() {
    ...
    let g = db.create_goal("Laptop".into(), 1000.0, None).unwrap();
    db.add_contribution(g.id.clone(), 250.0, None, None).unwrap();
    assert_eq!(db.list_goals().unwrap()[0].current_amount, 250.0);

    let d = db.create_debt("Card".into(), "credit".into(), 500.0, 500.0, 20.0, 25.0).unwrap();
    db.add_debt_payment(d.id, 100.0, None, None).unwrap();
    assert_eq!(db.list_debts().unwrap()[0].remaining_amount, 400.0);
